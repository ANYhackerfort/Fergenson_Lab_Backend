...
void ImageProcessor::preprocessForOCR() {
  ...
  // cv::Size(2, 2)); cv::morphologyEx(adaptiveThresholdImage,
  // adaptiveThresholdImage, cv::MORPH_OPEN, morphElement);

  // Perform dilation to make text bolder (optional for OCR)
  cv::dilate(adaptiveThresholdImage, preproccessed_ocr, cv::Mat(),
             cv::Point(-1, -1), 1);
}

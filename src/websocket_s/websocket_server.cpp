...
void do_session(tcp::socket socket) {
  try {
    ...
    void reply(tcp::socket socket) {
      ...
      std::string savePath = "/Users/matthewzhang/websocket-Fergenson/"
                             "test_data/processed_image.png";
      if (!cv::imwrite(savePath, gaussian_image)) {
        ws.write(asio::buffer("Failed to save processed image"));
        std::cerr << "Failed to save processed image to " << savePath
                  << std::endl;
        return;
      }
      ...
      std::cout
          << "Processed image saved to file, and confirmation message sent "
             "to the client."
          << std::endl;
    }
    catch (const std::exception &e) {
